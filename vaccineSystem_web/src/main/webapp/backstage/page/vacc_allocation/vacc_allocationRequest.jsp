...
<body>
    ...
    <script>
        layui.use(['form', 'jquery', 'table', 'layer', 'util','laydate'], function(){
            ...
            var t = table.render({
                elem: '#test'
                ,url: '/record/allocationView.action' //数据接口
            ...
            table.on('tool(test)', function(obj) {
                    ...
                    layer.open({
                        ...
                        success: function (layero, index) {//页面弹出成功触发
                            ...
                            form.on('select(bigType)', function (aaa) {
                                //console.log(data)
                                $.post("/allocation/getCity_number.action?city_table="+aaa.value+"&v_name="+data.vaccineName+"&v_spec="+data.vaccineSpec, function (data) {
                                    //判断业务响应码
                                    if (data.code == 200) {
                            ...
                            form.on("submit(submitFormFilter)", function (d) {
                                $.post("/allocation/updateV_number.action", d.field, function (rs) {
                                    //业务正常
                                    if (rs.code == 200) {
                                        layer.msg("调拨成功");
                ...
                function refByV_id(data) {
                    layer.confirm("确定拒绝调拨吗？", function (index) {
                        ...
                        $.post("//refuse.action?vaccineName="+vaccineName+"&vaccineSpec="+vaccineSpec, function (rs) {
                            //判断业务响应码
                            if (rs.code == 200) {
                                layer.msg("拒绝调拨成功");
