...
<body>
    ...
    layui.use(['form','table','jquery','layer','util'], function(){
        ...
        var t = table.render({
            elem: '#test'
            ,url: '/vaccine/showVaccineByCategory.action' //数据接口
        ...
        table.on('tool(test)', function(obj){
                ...
                layer.open({ //打开弹出层
                    ...
                    success: function (layero, index) {//页面弹出成功触发
                        ...
                        form.on("submit(submitFormFilter)", function (d) {
                            ...
                            $.post("/vaccine/editVaccine.action", d.field, function (rs) {
                                //业务正常
                                if (rs.code == 200) {
                                    layer.msg("更新成功");
            ...
            function delByV_id(data) {
                layer.confirm("确定删除选中的商品吗？", function (index) {
                    var v_id = data.v_id;
                    $.post("/vaccine/deleteByV_id.action?v_id="+v_id, function (rs) {
                        //判断业务响应码
                        if (rs.code == 200) {
                            layer.msg("删除成功");
